...
contract SupplyChain {
    ...
    modifier onlyOwner(uint32 _productId) {
        require(msg.sender == products[_productId].modelOwner, "Only owner can call this function");
        _; //flow continues after getting true above
    }
    ...
    function getParticipant(uint32 _id) public view returns (string memory, address, string memory) {
        return (participants[_id].userName,
            participants[_id].participantAddress,
            participants[_id].participantType);
    }
    ...
    function addProduct(uint32 _ownerId,
        ...
        if (keccak256(abi.encodePacked(participants[_ownerId].participantType)) == keccak256("Manufacturer")) {
            ...
            products[productId].modelOwner = participants[_ownerId].participantAddress;
    ...
    function getProduct(uint32 _productId) public view returns (string memory, string memory, string memory, uint32, address, uint32) {
        return (products[_productId].modelNumber,
            ...
            products[_productId].modelOwner,
            products[_productId].mfgTimeStamp);
    }

    function addOwner(uint32 _user1Id, uint32 _user2Id, uint32 _prodId) public onlyOwner(_prodId) returns (bool) {
        ...
        if (keccak256(abi.encodePacked(p1.participantType)) == keccak256("Manufacturer") &&
            ...
            products[_prodId].modelOwner = p2.participantAddress;
